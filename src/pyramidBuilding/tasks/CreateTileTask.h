...
namespace pb {
    ...
    explicit CreateTileTask(size_t numThreads) : ITask<BlockRequest < T>, Tile<T>>
...
void executeTask(std::shared_ptr<BlockRequest < T>>
...
copyTileBlock(newTileData, block[0]
.
...
copyTileBlock(newTileData, block[1]
.

get(), width, height, block[0]
...
copyTileBlock(newTileData, block[2]
.
...
copyTileBlock(newTileData, block[3]
.
...
<< "block was malformed. Size : " << block.
...
htgs::ITask<BlockRequest < T>, Tile <T> > *

copy()

override {
return new CreateTileTask(this->
...
T *generateDownsampleData(T *newTileData, size_t width, size_t height) {
    ...
    for (size_t i = 0; i < downsampleWidth - 1; i++) {
        ...
        downsampleData[index] = (newTileData[(height - 1) * width + 2 * i] + newTileData[(height - 2) * width + 2 * i] +
    ...
    for (size_t i = 0; i < downsampleHeight - 1; i++) {
        ...
        downsampleData[index] = (newTileData[width * 2 * i + width - 1] + newTileData[width * 2 * i + width - 2] +
    ...
    downsampleData[downsampleWidth * downsampleHeight - 1] =
            (newTileData[width * height - 1] + newTileData[width * (height - 1) - 1] + newTileData[width * height - 2] +
             newTileData[width * (height - 1) - 2]) / 4;
