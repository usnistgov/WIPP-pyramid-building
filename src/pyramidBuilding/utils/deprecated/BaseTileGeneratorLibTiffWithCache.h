 ...
 */
    ...
    public:
         ...
         */
        BaseTileGeneratorLibTiffWithCache(StitchingVectorParserOld *reader, BlendingMethod blendingMethod) : grid(reader->getGrid()), directory(
        ...
        Tile<T> *generateTile(std::pair<size_t, size_t> index) override {
            ...
            for (auto it2 = fovs.begin(); it2 != fovs.end(); ++it2) {
                ...
                for (auto y = overlapFov.y; y < overlapFov.y + overlapFov.height; y++) {
                    ...
                    blender->blend(image + y * fovWidth + overlapFov.x, overlapFov.width, tile + destOffset + (y - overlapFov.y) * pyramidTileWidth);
                }

                fovsCache->releaseFOV(fov->getFilename());
