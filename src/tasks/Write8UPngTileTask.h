...
#ifndef PYRAMIDBUILDING_WRITE8UPNGTILETASK_H
#define PYRAMIDBUILDING_WRITE8UPNGTILETASK_H
...
template <class T>
class Write8UPngTileTask : public WriteTileTask<T> {

public:


    Write8UPngTileTask(size_t numThreads, const std::string &_pathOut) :  WriteTileTask<T>(numThreads, _pathOut) {}

    Write8UPngTileTask(const std::string &_pathOut) : Write8UPngTileTask(1, _pathOut) {}
...
//        cv::Mat image(data->get_height(), data->get_width(), CV_32SC1, data->getData());
    ...
    ITask<Tile<T>, Tile<T>> *copy() override {
        return new Write8UPngTileTask(this->getNumThreads(), this->_pathOut);
    }

};

#endif //PYRAMIDBUILDING_WRITE8UPNGTILETASK_H
